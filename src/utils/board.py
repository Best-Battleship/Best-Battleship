...
def init_board(EMPTY, BOARD_SIZE):
...
def can_place_ship(board, ship_size, x, y, direction):
    if direction == 'H':
        return all(board[y][x+i] == EMPTY for i in range(ship_size) if x+i < BOARD_SIZE) and x+ship_size<BOARD_SIZE
    elif direction == 'V':
        return all(board[y+i][x] == EMPTY for i in range(ship_size) if y+i < BOARD_SIZE) and y+ship_size<BOARD_SIZE
    return False
